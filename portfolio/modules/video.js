...
class VideoControl {
    ...
    constructor(video, progressBar, volumeBar, volumeButton, playButton, bigPlayButton) {
        this.video = video;
    ...
    setupVideoEvents = () => {
        ...
        this.video.addEventListener('loadedmetadata', event => this.progressBar.max = Math.floor(this.video.duration));
    }
...
export function setupCustomControls(videoContainer) {
    ...
    let videoControl = new VideoControl(videoContainer.querySelector("video"), progress, volumeBar, volumeButton, playButton, bigPlayButton);
    volumeBar.value = 0.5;
